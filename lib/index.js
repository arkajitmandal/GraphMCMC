...
function getProbability(G1, G2, T) {
  ...
  return Math.min(1.0, Math.exp((theta1 - theta2) / T));
}
...
function proposeGraph(G) {
  ...
  while (badChoice) {
    ...
    let randomIndex = Math.floor(Math.random() * newGraph.Edges.length);
    newGraph.edgeRemove(randomIndex);
    if (newGraph.isConnected()) {
      badChoice = false;
      break;
    }
    let randomNode1 = Math.floor(Math.random() * newGraph.Nodes.length);
    let randomNode2 = Math.floor(Math.random() * newGraph.Nodes.length);
    while (randomNode1 === randomNode2) {
      randomNode2 = Math.floor(Math.random() * newGraph.Nodes.length);
    }
    let newEdge = new Edge(newGraph.Nodes[randomNode1], newGraph.Nodes[randomNode2]);
